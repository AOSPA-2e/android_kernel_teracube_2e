...
static u8 imx286_primax_otp_buf[OTP_DATA_SIZE];

struct imx286_write_buffer {
	u8 addr[2];
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX286_SENSOR_ID,	/* Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h */
...
{
	struct imx286_write_buffer buf;
	int i;
	int ret;
...
#endif
static int imx286_read_otp(u16 addr, u8 *buf)
{
	int ret = 0;
#if (CONFIG_OTP_FUNC)	
	...
	if (ret < 0)
		LOG_INF("read data from imx286 primax otp e2prom failed!\n");
#endif
	return ret;
}
...
static void read_imx286_pdaf_data(kal_uint16 addr, BYTE *data, kal_uint32 size)
{
	memcpy((void *)data, (void *)(imx286_primax_otp_buf + (pdaf_cal_data_offset - OTP_START_ADDR)),
	       PDAF_CAL_DATA_SIZE);
}
...
static void imx286_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
static void imx286_set_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	int i, idx;
...
#define IMX286MIPI_MaxGainIndex (255)

kal_uint16 IMX286MIPI_sensorGainMapping[IMX286MIPI_MaxGainIndex][2] = {
	{64, 0},
	{65, 6},
	{66, 12},
	{67, 20},
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (iI = 0; iI < IMX286MIPI_MaxGainIndex; iI++) {
		if (gain <= IMX286MIPI_sensorGainMapping[iI][0])
			return IMX286MIPI_sensorGainMapping[iI][1];
	}
	LOG_INF("exit IMX286MIPI_sensorGainMapping function\n");
	return IMX286MIPI_sensorGainMapping[iI-1][1];
}
#endif
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0409,0x20);
	write_cmos_sensor(0x040a,0x00);
	write_cmos_sensor(0x040b,0x14);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			ret = imx286_read_otp(0x0004, &module_id);
			if ((ret >= 0) && (*sensor_id == imgsensor_info.sensor_id)) {
				LOG_INF
				    ("Get imx286 sensor success!i2c write id: 0x%x, sensor id: 0x%x, module_id:0x%x\n",
				     imgsensor.i2c_write_id, *sensor_id, module_id);
				*sensor_id = IMX286_SENSOR_ID;

				pdaf_cal_data_offset = PDAF_CAL_DATA_OFFSET_C1;
				spc_start_addr = SPC_START_ADDR_C1;

				/* main_module_id = IMX286_SENSOR_ID; */
				goto otp_read;
			}
...
#if (CONFIG_OTP_FUNC)	
	imx286_read_otp_burst(OTP_START_ADDR, imx286_primax_otp_buf);

	/*Get AF infinity and macro position value */
	/* AF_Inf_pos   = (imx286_primax_otp_buf[67]<<8)|imx286_primax_otp_buf[68]; */
	/* AF_Macro_pos = (imx286_primax_otp_buf[71]<<8)|imx286_primax_otp_buf[72]; */
#if 0
	for (i = 0; i < OTP_DATA_SIZE; i++) {
		LOG_INF("========imx286_otp idx:0x%4x val:0x%x======\n", i,
			*(imx286_primax_otp_buf + i));
	}
#endif
#endif
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_DATA:
		...
		read_imx286_pdaf_data((kal_uint16) (*feature_data),
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		...
		imx286_get_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32), feature_data_16);
		break;
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		LOG_INF("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d", (*feature_para_len));
		imx286_set_pdaf_reg_setting((*feature_para_len) / sizeof(UINT32), feature_data_16);
		break;
		/******************** PDAF END   <<< *********/
...
UINT32 IMX286_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      IMX286_MIPI_RAW_SensorInit      */
