...
#define BIRD_OV9762_MIRROR

static struct imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV9762MIPI_SENSOR_ID,//OV9762MIPI_SENSOR_ID
	
	.checksum_value = 0x60b3d6ed,//0x1ec5153d cjc0428 chunmei
	...
	.ae_sensor_gain_delay_frame = 1, //1,//sensor gain delay frame for AE cycle,2 frame with ispGain_delay-sensor_gain_delay=2-0=2
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 1280, 960,    0,0, 1280,  960, 1280,  960, 8, 8, 1280, 960,	  0,	0, 1280, 960}, //hight speed video 
...
write_cmos_sensor(0x0340, 0x04); //Total vertical size[15:8]   frame      1072
write_cmos_sensor(0x0341, 0x30); // Total vertical size[7:0]
write_cmos_sensor(0x0342, 0x07); // Total horizontal size[15:8]   line    1864
write_cmos_sensor(0x0343, 0x48); //Total horizontal size[7:0]
write_cmos_sensor(0x0344, 0x00); //x_add_start[11:8]
write_cmos_sensor(0x0345, 0x38); //x_add_start[7:0] 
write_cmos_sensor(0x0346, 0x00); //y_add_start[11:8]
write_cmos_sensor(0x0347, 0x08); //y_add_start[7:0]  
write_cmos_sensor(0x0348, 0x05); //x_add_end[11:8]		
write_cmos_sensor(0x0349, 0x3F); //x_add_end[7:0]       1287
write_cmos_sensor(0x034A, 0x03); //y_add_end[11:8]	
write_cmos_sensor(0x034B, 0xCF); //y_add_end[7:0]       967
write_cmos_sensor(0x034C, 0x05); //X_OUTPUT_SIZE[11:8]  
write_cmos_sensor(0x034D, 0x00); //X_OUTPUT_SIZE[7:0]       1280
write_cmos_sensor(0x034E, 0x03); //y_OUTPUT_SIZE[11:8]  
write_cmos_sensor(0x034F, 0xC0); //y_OUTPUT_SIZE[7:0]      960
...
#if 1 //mclk=12mhz sclk=60mhz
write_cmos_sensor(0x3081, 0x02);	//PLL
write_cmos_sensor(0x3090, 0x01);	
write_cmos_sensor(0x3091, 0x28);//27->28	
...
#else
//mclk=24mhz sclk=60mhz
write_cmos_sensor(0x3081, 0x02);	
write_cmos_sensor(0x3090, 0x02); //PLL2
write_cmos_sensor(0x3091, 0x1e);	
write_cmos_sensor(0x3092, 0x03);
write_cmos_sensor(0x3094, 0x00);	
write_cmos_sensor(0x3098, 0x03);	
write_cmos_sensor(0x3099, 0x1e);	
write_cmos_sensor(0x309a, 0x04);	
write_cmos_sensor(0x30b3, 0x60); //pll1
write_cmos_sensor(0x30b4, 0x03);	
write_cmos_sensor(0x30b0, 0x0a);
write_cmos_sensor(0x30b1, 0x01);
#endif
write_cmos_sensor(0x3503, 0x27);	//0x27->0x07 gain not delay
...
write_cmos_sensor(0x3813, 0x04);	//vertical offset
...
write_cmos_sensor(0x4000, 0x43);//BLC	
write_cmos_sensor(0x4004, 0xc8);	
write_cmos_sensor(0x4008, 0x42);	
write_cmos_sensor(0x404f, 0x8f);	

write_cmos_sensor(0x4513, 0x60);//FIFO	
write_cmos_sensor(0x4520, 0xb0);	
write_cmos_sensor(0x4837, 0x15);	//PCLK_PERIOD
write_cmos_sensor(0x4d07, 0x04);	//OTP
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
   ...
   // added by xen for test 20150612,only back camera use this driver
   #if defined(OV9762_ONLY_AS_BACKCAMERA)
        if (m_SensorSel==1)
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	LOG_INF("feature_id = %d, feature_data=%x, para_len=%x\n", feature_id, (UINT32)*feature_data, *feature_para_len);
	switch (feature_id) {
...
UINT32 OV9762MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
