	...
	for (i = 0 ; i < MTK_VPU_CORE ; i++) {
		if (force_change_dsp_freq[i]) {
			...
			case 1:
				...
				break;
			case 2:
				LOG_DBG("thermal force bound freq @606MHz\n");
				break;
			case 3:
...
#undef DEFINE_APU_STEP
		...
		LOG_DBG("[vpu]init vvpu, vmdla to opp2\n");
		ret = vmdla_regulator_set_mode(true);
		udelay(100);
		LOG_DBG("vvpu set sleep mode ret=%d\n", ret);
...
int vpu_hw_processing_request(int core, struct vpu_request *request)
{
	...
	*/
	...
	ktime_get_ts(&start);
	if (g_vpu_log_level > Log_ALGO_OPP_INFO)
	LOG_DBG("[0x%lx]:d2d start ", (unsigned long)request->request_id);
	/* 3. wait until done */
	ret = wait_command(core);
	if (ret == -ERESTARTSYS)
		is_hw_fail = false;
