...
static int write_I2C_CAM_CAL(u16 a_u2Addr, u32 ui4_length, u8 *a_puBuff)
{
	int i4RetValue = 0;
	int idx, num;
	char puReadCmd[EEPROM_I2C_READ_MSG_LENGTH_MAX+2] = {  };
	//struct i2c_msg msg[EEPROM_I2C_MSG_SIZE_READ];
...
#endif
	num = 0;
	puReadCmd[num++] = (char)(a_u2Addr >> 8);
	puReadCmd[num++] = (char)(a_u2Addr & 0xFF);
	for (idx = 0; idx < ui4_length; idx++)
		puReadCmd[num++] = a_puBuff[idx];

	i4RetValue = i2c_master_send(g_pstI2CclientG, puReadCmd, num);
	spin_lock(&g_spinLock);
	g_pstI2CclientG->addr = g_pstI2CclientG->addr & I2C_MASK_FLAG;
	spin_unlock(&g_spinLock);
	if (i4RetValue < 0) {
