...
INT32 mtk_wcn_consys_jtag_set_for_mcu(VOID)
{
	...
	if (gJtagCtrl) {
		...
		case 1:
			...
			writel(tmp, addr);
			tmp = readl(addr);
			WMT_PLAT_PR_LOUD("(RegAddr, RegVal):(0x%p, 0x%x)\n", addr, tmp);
		...
		case 2:
			...
			break;
		default:
			WMT_PLAT_PR_LOUD("unsupported options!\n");
		}
...
error:
	...
	return ret;
#else
	WMT_PLAT_PR_LOUD("No support switch to JTAG!\n");
	...
	if (enable) {
		...
		while (consys_ver_id != 0x1D1E) {
			if (cnt > 10)
				break;
			consys_ver_id = CONSYS_REG_READ(conn_reg.mcu_base + 0x600);
			WMT_PLAT_PR_LOUD("0x18002600(0x%x)\n", consys_ver_id);
			WMT_PLAT_PR_LOUD("0x1800216c(0x%x)\n",
					CONSYS_REG_READ(conn_reg.mcu_base + 0x16c));
			WMT_PLAT_PR_LOUD("0x18007104(0x%x)\n",
			...
			if ((CONSYS_REG_READ(check_coredump_reg) == 0)
				...
				while (retry-- > 0) {
					...
					if (check_sleep == MCU_SLEEP_DONE) {
						WMT_PLAT_PR_LOUD("consys is ready to sleep\n");
						break;
					}
					WMT_STEP_DO_ACTIONS_FUNC
							(STEP_TRIGGER_POINT_POWER_OFF_HANDSHAKE);
					WMT_PLAT_PR_LOUD("check_sleep_reg=(0x%x)\n", check_sleep);
					msleep(20);
				}
...
static INT32 polling_consys_chipid(VOID)
{
	...
	WMT_PLAT_PR_LOUD("consys configuration id(0x%x)\n", consys_ver_id & 0xF);
...
static VOID consys_acr_reg_setting(VOID)
{
	WMT_PLAT_PR_LOUD("No need to do acr");
}

static VOID consys_afe_reg_setting(VOID)
{
	WMT_PLAT_PR_LOUD("No need to do afe");
}
...
static INT32 consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	if (enable) {
		...
		WMT_PLAT_PR_LOUD("turn on vcn33_2 for fm/gps usage in co-clock mode\n");
	} else {
		...
		WMT_PLAT_PR_LOUD("turn off vcn33_2 for fm/gps usage in co-clock mode\n");
	}
#endif
#endif
...
static UINT32 consys_emi_set_remapping_reg(VOID)
{
	...
	CONSYS_REG_WRITE_OFFSET_RANGE(conn_reg.topckgen_base + CONSYS_EMI_PERI_MAPPING_OFFSET,
					  0x10003000, 0, 16, 20);
	WMT_PLAT_PR_LOUD("PERISYS_MAPPING dump in restore cb(0x%08x)\n",
	...
	if (size == 0)
		WMT_PLAT_PR_LOUD("MD direct path is not supported\n");
	else {
		CONSYS_REG_WRITE_OFFSET_RANGE(
			conn_reg.topckgen_base + CONSYS_EMI_AP_MD_OFFSET,
			mdPhy, 0, 16, 20);
		WMT_PLAT_PR_LOUD("MD_MAPPING dump in restore cb(0x%08x)\n",
...
static INT32 consys_dedicated_log_path_init(struct platform_device *pdev)
{
	...
	if (node) {
		...
		irq_flag = irq_get_trigger_type(irq_num);
		WMT_PLAT_PR_LOUD("get conn2ap_sw_irq id(%d) and irq trigger flag(%d) from DT\n",
				irq_num, irq_flag);
	} else {
		WMT_PLAT_PR_ERR("[%s] can't find CONSYS compatible node\n", __func__);
		return iret;
...
static INT32 consys_reset_emi_coredump(UINT8 __iomem *addr)
{
	...
	WMT_PLAT_PR_LOUD("Reset EMI(0xF0068000 ~ 0xF0068400) and (0xF0070400 ~ 0xF0078400)\n");
...
static VOID consys_set_mcif_emi_mpu_protection(MTK_WCN_BOOL enable)
{
	WMT_PLAT_PR_LOUD("Setup region 23 for domain 0 as %s\n", enable ? "FORBIDDEN" : "SEC_R_NSEC_R");
