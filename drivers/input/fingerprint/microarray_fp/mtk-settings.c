...
void ma_spi_change(struct spi_device *spi, unsigned int  speed, int flag)
{
   /* struct mt_chip_conf *mcc = (struct mt_chip_conf *)spi->controller_data;
    ...
    mcc->low_time = speed;
    if(spi_setup(spi) < 0){
        printk("dantangwei1change the spi error!\n");
    }*/    
}
...
int mas_get_platform(void) {
	...
	if(ret){
		printk("dantangwei1platform_driver_register");
	}
    /*printk("dantangweistart board info");
	ret = spi_register_board_info(smt_info, ARRAY_SIZE(smt_info));
	if(ret){
		printk("dantangwei1spi_register_board_info");
	}*/
  
    printk("dantangweistart register spi");
	ret = spi_register_driver(&sdrv);
	if(ret) {
		printk("dantangwei1spi_register_driver");
	}
	return ret;
}
	...
	switch(cmd)
	{
		case 0:
			if( (!IS_ERR(mas_spi_cs_off)) & (!IS_ERR(mas_spi_ck_off)) & (!IS_ERR(mas_spi_mi_off)) & (!IS_ERR(mas_spi_mo_off)) ){
				...
				printk("dantangwei1mas_spi_gpio_slect_pinctrl cmd=0 err!");
				return -1;
			}
			break;
		case 1:
			if( (!IS_ERR(mas_spi_cs_on)) & (!IS_ERR(mas_spi_ck_on)) & (!IS_ERR(mas_spi_mi_on)) & (!IS_ERR(mas_spi_mo_on)) ){
				...
				printk("dantangwei1mas_spi_gpio_slect_pinctrl cmd=1 err!");
				return -1;
			}
			break;
	}
	...
	switch (cmd)
		...
		case 0 : 		
			if( (!IS_ERR(mas_finger_power2v8_off)) & (!IS_ERR(mas_finger_power1v8_off)) ){
				...
				printk("dantangwei1mas_power_gpio_slect_pinctrl cmd=0 err!");
				return -1;
			}
		break;
		case 1 : 		
			if( (!IS_ERR(mas_finger_power2v8_on)) & (!IS_ERR(mas_finger_power1v8_on)) ){
				...
				printk("dantangwei1mas_power_gpio_slect_pinctrl cmd=1 err!");
				return -1;
			}
		break;
		}
	...
	switch (cmd)
		...
		case 0 : 		
			if(!IS_ERR(mas_finger_eint_off)){
				...
				printk("dantangwei1mas_eint_gpio_slect_pinctrl cmd=0 err!");
				return -1;
			}		
			break;
		case 1 : 		
			if(!IS_ERR(mas_finger_eint_on)){
				...
				printk("dantangwei1mas_eint_gpio_slect_pinctrl cmd=1 err!");
				return -1;
			}
			break;
		}
...
void mas_enable_spi_clock(struct spi_device *spi){
	...
	//printk("dantangweistart spi enable");
...
void mas_disable_spi_clock(struct spi_device *spi){
	...
	//printk("dantangweistart spi disable");
...
unsigned int mas_get_irq(struct device *dev){
    ...
    if (!gpio_is_valid(finger_int_pin)){
	    printk("dantangwei1invalid irq gpio!");
		return -EINVAL;
    }
