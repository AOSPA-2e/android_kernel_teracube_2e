...
#include "sp0821mipiraw_Sensor.h"

#define PFX "SP0821_camera_sensor"

#define LOG_1 LOG_INF("SP0821,MIPI 1LANE\n")
...
#if defined(YK668_CUSTOMER_CAIFU_S570_HD720)  //zlh 20170919
#define IMAGE_MIRROR_HV
#define IMAGE_MIRROR_R
#endif
	  ...
	  if(gain >= BASEGAIN && gain <= 15*BASEGAIN)
   	 {   
					...
					if(iReg >= 0x80 && imgsensor.shutter >= 0xb8d)
						{
							...
							LOG_INF("SP0821MIPI_SetGain 1111111111--0x1f = 0x%x\n",read_cmos_sensor(0x1f));
							LOG_INF("SP0821MIPI_SetGain 1111111111--0x20 = 0x%x\n",read_cmos_sensor(0x20));
							LOG_INF("SP0821MIPI_SetGain 1111111111--0x21 = 0x%x\n",read_cmos_sensor(0x21));
							LOG_INF("SP0821MIPI_SetGain 1111111111--0x28 = 0x%x\n",read_cmos_sensor(0x28));
							LOG_INF("SP0821MIPI_SetGain 1111111111--0x29 = 0x%x\n",read_cmos_sensor(0x29));
						}
						else
						{
							write_cmos_sensor(0xfd, 0x02);
   	    	 ...
   	    	 else if(iReg>= 0xa0)//gpw
   	    	 {
   	    	    	 ...
   	    	    	 write_cmos_sensor(0x01, 0x01); 
                      LOG_INF("SP0821MIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP0821MIPI_SetGain = %d",iReg);		 
	       }	
   	 }	
   	 else
...
{
write_cmos_sensor(0xfd,0x01);
#if defined(IMAGE_MIRROR_HV)//zlh for mirror
write_cmos_sensor(0x3f,0x03);
#endif
        ...
        do {
            *sensor_id = return_sensor_id();
            ...
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF(" gxb get i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id); 
                return ERROR_NONE;
            }    
            LOG_INF(" gxb get Read sensor id fail, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            sensor_id = return_sensor_id();
            if (sensor_id == imgsensor_info.sensor_id) {                
                LOG_INF(" gxb open i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);   
                break;
            }   
            LOG_INF(" gxb open  Read sensor id fail, write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 SP0821MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}   /*  SP0821_MIPI_RAW_SensorInit  */
