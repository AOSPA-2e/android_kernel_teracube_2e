...
#if defined(YK628_CUSTOMER_JIAHE_Z1_QHD_50)\
 || defined(YK628_CUSTOMER_GONGCHUANG_B_QHD_50) // xen 20150515,front camera
  #define GC2145_MIRROR_SET
#elif defined(YK606_CUSTOMER_JIAFENGDA_T8_FWVGA_50) // xen 20150610,back camera
  #define GC2145_MIRROR_SET
#endif
...
kal_uint32 GC2145MIPI_isp_master_clock=0;
...
static void GC2145MIPI_Sensor_Init(void)
{
    ...
    #if defined(GC2145_MIRROR_SET) // xen 20150515
	...
	GC2145MIPI_write_cmos_sensor(0x02, 0x77); //0x22); //driving current increase by xen 20170515
	GC2145MIPI_write_cmos_sensor(0x03, 0x17); //0x10); // 0x12 20140821
...
UINT32 GC2145MIPIGetInfo(enum MSDK_SCENARIO_ID_ENUM ScenarioId,
...
UINT32 GC2145MIPIControl(enum MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow,
					  MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	switch (ScenarioId)
	{
...
void GC2145MIPI_set_param_wb(UINT16 para)
{
	switch (para)
	{
		case AWB_MODE_AUTO:
...
void GC2145MIPI_set_param_effect(UINT16 para)
{
...
void GC2145MIPI_set_param_banding(UINT16 para)
{
    switch (para)
    {
...
void GC2145MIPI_set_param_exposure(UINT16 para)
{
	switch (para)
	{
...
UINT32 GC2145MIPIYUVSensorSetting(enum FEATURE_ID iCmd, UINT16 iPara)
{
...
/*************************************************************************
  ...
  *************************************************************************/
  ...
  UINT32 GC2145MIPISetMaxFramerateByScenario(enum MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	//kal_uint32 pclk;
	//kal_int16 dummyLine;
...
static void GC2145MIPISetTestPatternMode(kal_bool bEnable)
{
 printk("[GC2145MIPISetTestPatternMode] Test pattern enable:%d\n", bEnable);

 if(bEnable)	// enable test pattern output  0809 ok!!!
 {
	 // GC2145MIPI_write_cmos_sensor(0xfe,0x00);  
	 // GC2145MIPI_write_cmos_sensor(0x8c,0x02);

	 GC2145MIPI_write_cmos_sensor(0xfe, 0x00); 
	 //GC2145MIPI_write_cmos_sensor(0xfa, 0x11); 
	 GC2145MIPI_write_cmos_sensor(0x18, 0x06); //sdark
	 ...
	 GC2145MIPI_write_cmos_sensor(0xfe, 0x02); //
	 GC2145MIPI_write_cmos_sensor(0xd0, 0x40); //asde saturation
	 GC2145MIPI_write_cmos_sensor(0xdd, 0x00); //edge_sa
	 GC2145MIPI_write_cmos_sensor(0xfe, 0x00); //
	 GC2145MIPI_write_cmos_sensor(0x03, 0x00); //
	 GC2145MIPI_write_cmos_sensor(0x04, 0x00); //0x01
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_SET_YUV_CMD:
			...
			GC2145MIPIYUVSensorSetting((enum FEATURE_ID)*feature_data, *(feature_data+1));
		break;
...
static struct SENSOR_FUNCTION_STRUCT SensorFuncGC2145MIPI=
{
	GC2145MIPIOpen,
	GC2145MIPIGetInfo,
	GC2145MIPIGetResolution,
