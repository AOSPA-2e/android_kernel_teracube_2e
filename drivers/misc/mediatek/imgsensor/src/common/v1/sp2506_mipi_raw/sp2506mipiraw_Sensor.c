...
#include "sp2506mipiraw_Sensor.h"
...
#define PFX "sp2506_camera_primax"
#define LOG_INF_LOD(format, args...)    pr_info(PFX "[%s] " format, __func__, ##args)
#define LOG_1 LOG_INF("sp2506,MIPI 4LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP2506_SENSOR_ID,	/* Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h */

	.checksum_value = 0xf7375923,	/* checksum value for Camera Auto Test */

	.pre = {
        .pclk = 84000000,            //record different mode's pclk
		.linelength = 2262,	/* record different mode's linelength */
...
//	.custom3_delay_frame = 2,
	...
	.i2c_addr_table = {0x68, 0xff},	/* record sensor support all write id addr,0x68
...
static void write_shutter(kal_uint16 shutter)
{
        ...
        write_cmos_sensor(0xfe, 0x02);                 
        LOG_INF("sp250611111-------shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
	  ...
	  kal_uint8  iReg;
   	 ...
   	 {   
   	    	 ...
   	    	 else if(iReg>= 0xf8)//huazai
   	    	 {
   	  					...
   	  					write_cmos_sensor(0xfe, 0x02); 
                      LOG_INF("SP2506MIPI_SetGain = 160"); 
	        }        	
   	    	 else
   	    	 {
   	    	    	write_cmos_sensor(0xfd, 0x01);
   	    	    	...
   	    	    	write_cmos_sensor(0xfe, 0x02);
			LOG_INF("sp250611111-------SP2506MIPI_SetGain ireg = 0x%x,gain = 0x%x\n",iReg,gain);		 
	       }	
   	 }	
   	 else
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x23, 0x03);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x5f, 0x55);
	write_cmos_sensor(0x60, 0x55);
	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0x3a, 0x0d);
	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0x3b, 0x00);
	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0x3e, 0x01);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x61, 0x0e);
	write_cmos_sensor(0x62, 0xee);
	write_cmos_sensor(0x36, 0x01);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x56, 0x04);
	write_cmos_sensor(0x57, 0x20);
	write_cmos_sensor(0x30, 0x04);
	write_cmos_sensor(0x31, 0x20);
	write_cmos_sensor(0x58, 0x06);
	write_cmos_sensor(0x59, 0x1C);
	write_cmos_sensor(0x32, 0x06);
	write_cmos_sensor(0x33, 0x1C);
	write_cmos_sensor(0x35, 0x08);
	write_cmos_sensor(0x3f, 0x10);
	write_cmos_sensor(0x36, 0x1C);
	write_cmos_sensor(0x40, 0x1E);
	write_cmos_sensor(0x34, 0x02);
	write_cmos_sensor(0x37, 0x06);
	write_cmos_sensor(0x3d, 0x0b);
	write_cmos_sensor(0x3e, 0x0b);
	write_cmos_sensor(0x43, 0x02);
	write_cmos_sensor(0x44, 0x1A);
	write_cmos_sensor(0x45, 0x00);
	write_cmos_sensor(0x46, 0x1C);
	write_cmos_sensor(0x49, 0x10);
	write_cmos_sensor(0x4a, 0x1a);
	write_cmos_sensor(0x47, 0xD2);
	write_cmos_sensor(0x48, 0x42);
	write_cmos_sensor(0x3a, 0x7b);
	write_cmos_sensor(0x3b, 0x01);
	write_cmos_sensor(0x3c, 0x7E);
	write_cmos_sensor(0x4b, 0x17);
	write_cmos_sensor(0x4c, 0x0d);
	write_cmos_sensor(0x4e, 0xEF);
	write_cmos_sensor(0x4f, 0x03);
	write_cmos_sensor(0x50, 0x80);
	write_cmos_sensor(0x16, 0x00);
	write_cmos_sensor(0x17, 0xC7);
	write_cmos_sensor(0x1b, 0x3f);
	write_cmos_sensor(0x1d, 0x60);
	write_cmos_sensor(0xfd, 0x03);
	write_cmos_sensor(0x86, 0x03);
	write_cmos_sensor(0x82, 0x04);
	write_cmos_sensor(0x83, 0xbe);
	write_cmos_sensor(0x84, 0x00);
	write_cmos_sensor(0x85, 0x9f);
	write_cmos_sensor(0xfd, 0x00);
	write_cmos_sensor(0x34, 0x01);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x0a, 0x00);
	write_cmos_sensor(0x0e, 0x04);
	write_cmos_sensor(0x0f, 0x50);
	write_cmos_sensor(0xfd, 0x01);
				write_cmos_sensor(0x12, 0x00); //direction
	write_cmos_sensor(0x21, 0x00);
	write_cmos_sensor(0x22, 0x82);
	write_cmos_sensor(0xfe, 0x02);
	write_cmos_sensor(0xfd, 0x01);
	write_cmos_sensor(0x68, 0x1f);
	write_cmos_sensor(0x6a, 0x01);
	write_cmos_sensor(0x64, 0x04);
	write_cmos_sensor(0x65, 0xfe);
	write_cmos_sensor(0x5a, 0x01);
	write_cmos_sensor(0x5b, 0x02);
	write_cmos_sensor(0x5c, 0x01);
	write_cmos_sensor(0x5d, 0x0f);
	write_cmos_sensor(0x05, 0x14);
	write_cmos_sensor(0x09, 0xB0);
	write_cmos_sensor(0x03, 0x6C);
	write_cmos_sensor(0x07, 0x24);
	write_cmos_sensor(0x0d, 0x22);
	write_cmos_sensor(0x6c, 0x22);
	write_cmos_sensor(0x6f, 0x00);
	write_cmos_sensor(0x70, 0x7e);
	write_cmos_sensor(0x71, 0x02);                 
	write_cmos_sensor(0x72, 0x7d);
	write_cmos_sensor(0xfd, 0x03);
	write_cmos_sensor(0x88, 0x7D);
	write_cmos_sensor(0x89, 0x1d);
	write_cmos_sensor(0x9f, 0x02);
	write_cmos_sensor(0x03, 0x22);
	write_cmos_sensor(0x04, 0x74);
	write_cmos_sensor(0x05, 0x22);
	write_cmos_sensor(0x06, 0x76);
	write_cmos_sensor(0x07, 0xdc);
	write_cmos_sensor(0x08, 0x78);
	write_cmos_sensor(0x09, 0x30);
	write_cmos_sensor(0x0a, 0x96);
	write_cmos_sensor(0x0b, 0x30);
	write_cmos_sensor(0x0c, 0xa0);
	write_cmos_sensor(0x0d, 0x30);
	write_cmos_sensor(0x18, 0xff);
	write_cmos_sensor(0x19, 0x48);
	write_cmos_sensor(0x1b, 0x0b);
	write_cmos_sensor(0x1a, 0x78);
	write_cmos_sensor(0x1d, 0xff);
	write_cmos_sensor(0x1c, 0xff);
	write_cmos_sensor(0x1e, 0xff);
	write_cmos_sensor(0x1f, 0xff);
	...
	write_cmos_sensor(0x98, 0x87);
	write_cmos_sensor(0x99, 0x00);
	write_cmos_sensor(0x9a, 0x87);
	write_cmos_sensor(0x9b, 0x00);
	write_cmos_sensor(0x9c, 0x87);
	write_cmos_sensor(0x9d, 0x00);
	write_cmos_sensor(0x9e, 0x87);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("Get sp2506 sensor success!i2c write id: 0x%x, sensor id: 0x%x\n",imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			if (sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("sp2506 2015_12_24 i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
...
UINT32 sp2506_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      sp2506_MIPI_RAW_SensorInit      */
