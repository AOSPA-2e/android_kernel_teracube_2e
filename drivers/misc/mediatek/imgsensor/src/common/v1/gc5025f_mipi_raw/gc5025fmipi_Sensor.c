...
#include "gc5025fmipi_Sensor.h"


#undef GC5025H_MIRROR_NORMAL
#undef GC5025H_MIRROR_H
#undef GC5025H_MIRROR_V
...
#if defined(GC5025HOTP_FOR_CUSTOMER)
#define RG_TYPICAL          1233
#define BG_TYPICAL          1198
...
#define LOG_1 pr_debug("GC5025H,MIPI 2LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
       .sensor_id = GC5025FMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

       .checksum_value = 0x1c10cbe1,//0xf7375923,//0x56d1c29c,//0xf7375923,        //checksum value for Camera Auto Test
...
static struct gc5025h_otp gc5025h_otp_info;
...
static void gc5025h_gcore_read_otp_info(void)
{
	...
	kal_uint8 golden[5];
#endif
	memset(&gc5025h_otp_info, 0, sizeof(gc5025h_otp_info));
	...
	case 0x01:
		...
		for (i = 0; i < total_number; i++) {
			...
			if (ddtempbuff[4 * i + 3] & 0x10) {
				...
				case 3:
					for (j = 0; j < 4; j++) {
						gc5025h_otp_info.dd_param_x[cnt] =
							(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8)
							+ ddtempbuff[4 * i] - ddoffsetX;
						gc5025h_otp_info.dd_param_y[cnt] =
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x01:
			...
			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc5025h_otp_info.rg_gain = (((wb[0] << 8) & 0xff00) | wb[1]) > 0 ?
					(((wb[0] << 8) & 0xff00) | wb[1]) : 0x400;
				gc5025h_otp_info.bg_gain = (((wb[2] << 8) & 0xff00) | wb[3]) > 0 ?
		...
		case 0x01:
			...
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc5025h_otp_info.golden_rg = (((golden[0] << 8) & 0xff00) | golden[1]) > 0 ?
					(((golden[0] << 8) & 0xff00) | golden[1]) : RG_TYPICAL;
				gc5025h_otp_info.golden_bg = (((golden[2] << 8) & 0xff00) | golden[3]) > 0 ?
	...
	if (0x01 == gc5025h_otp_info.dd_flag) {
		...
		for (i = 0; i < gc5025h_otp_info.dd_cnt - 1; i++)
			for (j = i + 1; j < gc5025h_otp_info.dd_cnt; j++)
				if (gc5025h_otp_info.dd_param_y[i] * WINDOW_WIDTH
					+ gc5025h_otp_info.dd_param_x[i]
					> gc5025h_otp_info.dd_param_y[j] * WINDOW_WIDTH
					+ gc5025h_otp_info.dd_param_x[j]) {
					temp_x = gc5025h_otp_info.dd_param_x[i];
					gc5025h_otp_info.dd_param_x[i] = gc5025h_otp_info.dd_param_x[j];
					gc5025h_otp_info.dd_param_x[j] = temp_x;
					temp_y = gc5025h_otp_info.dd_param_y[i];
					gc5025h_otp_info.dd_param_y[i] = gc5025h_otp_info.dd_param_y[j];
					gc5025h_otp_info.dd_param_y[j] = temp_y;
					temp_type = gc5025h_otp_info.dd_param_type[i];
					gc5025h_otp_info.dd_param_type[i] = gc5025h_otp_info.dd_param_type[j];
					gc5025h_otp_info.dd_param_type[j] = temp_type;
				}
		...
		for (i = 0; i < gc5025h_otp_info.dd_cnt; i++) {
			...
			temp_val1 = (((gc5025h_otp_info.dd_param_y[i]) << 4) & 0x00f0)
						+ ((gc5025h_otp_info.dd_param_x[i] >> 8) & 0X000f);
			...
			while ((i < gc5025h_otp_info.dd_cnt - 1)
				&& (gc5025h_otp_info.dd_param_x[i] == gc5025h_otp_info.dd_param_x[i + 1])
				&& (gc5025h_otp_info.dd_param_y[i] == gc5025h_otp_info.dd_param_y[i + 1])) {
					flag = 1;
					i++;
				}
			if (flag)
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x01, 0x47);//cjc 0x07-> 0x47
	write_cmos_sensor(0xfc, 0x2e);
	write_cmos_sensor(0xfe, 0x00);
	...
	write_cmos_sensor(0x17, GC5025H_MIRROR);
	...
	write_cmos_sensor(0x8b, 0x31); //0x39-->0x31 ITS:test_black_white
	write_cmos_sensor(0xfe, 0x01);
	...
	write_cmos_sensor(0x92, GC5025H_STARTY);
	write_cmos_sensor(0x94, GC5025H_STARTX);

	/* MIPI */
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x02, 0x04);//cjc 0x03-> 0x04
    ...
    kal_uint8 retry = 2;
		...
		//if(m_SensorSel == 1) //xjl 20180615
		    //*sensor_id = GC5025MIPI_SENSOR_ID;
            	...
            	if (*sensor_id == GC5025MIPI_SENSOR_ID)
            	   *sensor_id = GC5025FMIPI_SENSOR_ID;

            	if (*sensor_id == imgsensor_info.sensor_id) {
					gc5025h_gcore_identify_otp();
					if(gc5025h_otp_info.module_id!=0x05)
					{
					...
					{
                	pr_debug("gc5025 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                	return ERROR_NONE;

					}
            	}
            	pr_debug("gc5025 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            	retry--;
        	} while(retry > 0);
        	i++;
        	retry = 2;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
	    ...
	    //if(m_SensorSel == 1) //xjl 20180615
            ...
            if (sensor_id == imgsensor_info.sensor_id) {
					...
					{
				    pr_debug("gc5025 gc5025h_otp_info.module_id  : 0x%x\n", gc5025h_otp_info.module_id);
...
UINT32 GC5025FMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
