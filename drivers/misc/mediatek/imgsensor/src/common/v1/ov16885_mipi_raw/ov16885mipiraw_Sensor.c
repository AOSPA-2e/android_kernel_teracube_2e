 ...
 *   setting updata: Ov16885R1A_AM23.db
 *
 *------------------------------------------------------------------------------
...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.mipi_settle_delay_mode = 1,	/* 0,MIPI_SETTLEDELAY_AUTO;
					 ...
					 */
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_R,//sensor output first pixel color
...
static void capture_setting(kal_uint16 currefps)
{
    ...
    *; V-blank:4.1ms			
	...
	write_cmos_sensor(0x0301, 0xa5);
	write_cmos_sensor(0x0304, 0x3c);
	write_cmos_sensor(0x0316, 0x80);
	write_cmos_sensor(0x3501, 0xec);
	write_cmos_sensor(0x3511, 0xec);
	write_cmos_sensor(0x3600, 0x00);
	write_cmos_sensor(0x3602, 0x86);
...
/*
	...
	* ; Sysclk 160Mhz, MIPI4_576Mbps/Lane, 60Fps.
    *;Line_length =1040, Frame_length =1282
    *; V-blank:1.3ms
	*/
	
	write_cmos_sensor(0x0301, 0xa5);
	write_cmos_sensor(0x0304, 0x32);
	write_cmos_sensor(0x0316, 0xa0);
	write_cmos_sensor(0x3501, 0x4f);
	write_cmos_sensor(0x3511, 0x4f);
	write_cmos_sensor(0x3600, 0x00);
	write_cmos_sensor(0x3602, 0x82);
	write_cmos_sensor(0x3621, 0x88);
	write_cmos_sensor(0x366c, 0x03);
	...
	write_cmos_sensor(0x3801, 0xa8);
	write_cmos_sensor(0x3802, 0x02);
	write_cmos_sensor(0x3803, 0xa8);
	write_cmos_sensor(0x3804, 0x10);
	write_cmos_sensor(0x3805, 0xb7);
	...
	write_cmos_sensor(0x3834, 0xf0);
	write_cmos_sensor(0x3f03, 0x10);
	write_cmos_sensor(0x3f05, 0x28);
	write_cmos_sensor(0x4013, 0x14);
	write_cmos_sensor(0x4014, 0x08);
	write_cmos_sensor(0x4016, 0x11);
	write_cmos_sensor(0x4018, 0x07);
	write_cmos_sensor(0x4500, 0xc0);
	write_cmos_sensor(0x4501, 0x01);
	write_cmos_sensor(0x4503, 0x23);
...
{
	LOG_INF("enable: %d\n", enable);
	/* test pattern registers of ov16885, address 0x5081, default value is 0x00
		desc:
		Bit[7]: Flip option
		Bit[6]: Mirror option
		Bit[5:4]: Color bar option
			Range is [0:3]
		Bit[3]: Transparent mode enable
		Bit[2]: Rolling bar enable
		Bit[1]: Clear last four bits enable
		Bit[0]: Test pattern enable */
	if (enable) {
